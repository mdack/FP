	...
	while (op != 0){
		...
		case 2:
			if (nuevaPartida(gestor)){ cout << endl << "Se ha creado una nueva partida" << endl << endl; }
				...
				if (gestor.listUsers.users[gestor.usuario]->espera)
...
void mostrarPartidasEnCurso(tGestor & gestor){
	...
	cout << "---------------------------------------------------------------------------\n";
	for (int i = 1; i <= partidasUsuario(gestor); i++){
		cout << " " << i << ".   ";
...
void elegirPartida(tGestor & gestor){
	...
	cin >> op;
	while (cin.fail() || op > partidasUsuario(gestor) || op < 0){
		cin.clear();
		cin.sync();
		cout << "Respuesta no valida" << endl;
...
void menuPartida(tGestor & gestor){
	...
	int col;
		...
		{
		case 1:
			if (esSuTurno(gestor, *(gestor.partida))){
				leerJugada(col);
